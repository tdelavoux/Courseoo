<?php
	...
	class IngredientRecette extends \data\Data
	{
        ...
        * ###############################################################################*/
        ...
        public function deleteIngredient($fkIngredient)
        {
            $statement = $this->db->prepare(
                "DELETE FROM  IngredientRecette
                WHERE IngredientRecette.id = :fkIngredient");
            $statement->bindParam(':fkIngredient', $fkIngredient, \PDO::PARAM_INT);
            $statement->execute();
        }
	}
